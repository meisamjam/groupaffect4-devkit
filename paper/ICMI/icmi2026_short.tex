%% icmi2026_short.tex  (GroupAffect-4 modelling devkit / technical report)
%% Title: GroupAffect-4 Devkit
%%
%% Compile:
%%   pdflatex icmi2026_short
%%   bibtex   icmi2026_short
%%   pdflatex icmi2026_short
%%   pdflatex icmi2026_short
...
\begin{itemize}[leftmargin=1.2em,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{(M3) Preprocessing, window-length, and modality ablation.}
    ...
    IMU unsmoothed --- combined with AP1 augmentation, yields the strongest
    balanced result: $V{=}0.600$, $A{=}0.624$, $D{=}0.436$, improving all
...
For each modality $m \in \{\text{gaze, pupil, EDA, PPG, IMU}\}$, a two-layer
...
Combined with AP1 augmentation, this yields the strongest balanced result:
...
The per-dimension optimal configs differ: Conv1D+SimCLR+TW for Valence,
Conv1D+SimCLR+default for Arousal, GRU+SimCLR+no-aug for Dominance.
...
\textbf{Deployment guidance:} Physio-smooth centered + AP1 is the recommended
default --- it improves all three dimensions and retains $N{=}103$.
...
$V{=}0.600$, $A{=}0.624$, $D{=}0.436$ &
Strongest balanced 15s-compatible recipe; preserves $N{=}103$. \\
\addlinespace
...
gaze/pupil/IMU raw) combined with AP1 achieves the strongest balanced
