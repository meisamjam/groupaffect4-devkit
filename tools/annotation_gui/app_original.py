...
from .media.video_engine import VideoPanel, _is_cam5_feed, _is_tobii_feed
...
class MainWindow(QMainWindow):
    def __init__(self, bids_root: Path | None = None) -> None:
        ...
        self._active_word_row: int | None = None
    ...
    def _build_ui(self) -> None:
        ...
        right_layout.addWidget(self.tier_panel, 2)
        ...
        right_layout.addWidget(self.transcript_path_label)

        self.transcript_editor = QTextEdit()
        self.transcript_editor.setPlaceholderText("Load transcript, edit here, then Save.")
        right_layout.addWidget(self.transcript_editor, 2)

        transcript_tables_header = QHBoxLayout()
        transcript_tables_header.addWidget(QLabel("Transcript Rows (editable speaker/text)"))
        ...
        right_layout.addLayout(transcript_tables_header)

        self.transcript_segments_table = QTableWidget(0, 4)
        self.transcript_segments_table.setHorizontalHeaderLabels(["Time", "Speaker", "Mic", "Text"])
        ...
        right_layout.addWidget(self.transcript_segments_table, 2)
        ...
        right_layout.addWidget(self.transcript_words_table, 2)
    ...
    def _setup_videos(self, tr: TaskRun) -> None:
        cols = 3
        default_cam5_mask = [_is_cam5_feed(mf.acq or mf.path.stem) for mf in tr.video]
        has_cam5 = any(default_cam5_mask)
        for i, mf in enumerate(tr.video):
            ...
            cb = QCheckBox(label)
            default_on = default_cam5_mask[i] if has_cam5 else i == 0
    ...
    def _update_transcript_playhead(self, playhead_seconds: float) -> None:
        ...
        active_word_idx = _find_active_transcript_row(self._word_rows, transcript_t)
        if active_word_idx is not None:
            row = self._word_rows[active_word_idx]
            speaker = str(row.get("speaker", "-") or "-")
            word = str(row.get("word", "") or "")
            mic = str(row.get("mic", "") or "")
            self.speaker_strip_label.setText(f"Now speaking: {speaker}")
            self.word_strip_label.setText(f"Word: {word} [{mic}]" if mic else f"Word: {word}")
            if active_word_idx != self._active_word_row and active_word_idx < self.transcript_words_table.rowCount():
                self._active_word_row = active_word_idx
                self.transcript_words_table.selectRow(active_word_idx)
                self.transcript_words_table.scrollToItem(
                    self.transcript_words_table.item(active_word_idx, 0),
        ...
        active_segment_idx = _find_active_transcript_row(self._segment_rows, transcript_t)
        if (
            active_segment_idx is not None
            and active_segment_idx != self._active_segment_row
            and active_segment_idx < self.transcript_segments_table.rowCount()
        ):
            self._active_segment_row = active_segment_idx
            self.transcript_segments_table.selectRow(active_segment_idx)
...
def _load_audio_annot_transcript(
        ...
        try:
            with master_transcript.open("r", encoding="utf-8", newline="") as f:
                ...
                for row in reader:
                    ...
                    lines.append(f"[{ts}] {speaker}{mic_part}: {text}")
    ...
    if master_words.is_file():
        ...
        try:
            ...
            lines.append("")
            lines.append(f"# master_words load error: {type(exc).__name__}: {exc}")
        else:
            lines.append("")
            lines.append(f"# master_words summary: {word_count} words")
            if by_mic:
                mic_summary = ", ".join(f"{k}={v}" for k, v in sorted(by_mic.items()))
                lines.append(f"# words by mic: {mic_summary}")
...
def _find_active_transcript_row(rows: list[dict[str, object]], t_seconds: float) -> int | None:
...
def _write_tsv_rows(path: Path, rows: list[dict[str, object]]) -> None:
    if not rows:
        return
    headers = [k for k in rows[0].keys() if not str(k).startswith("_")]
